...
from .utils.utils import (read_image, read_overlap_image, visualize_overlap,
                          visualize_overlap_crop, ensure_prediction_fields)
...
def process(
    ...
    pred = ensure_prediction_fields(pred, data)

    if 'ratio0' in pred:
        ratio0 = pred['ratio0']
        ratio1 = pred['ratio1']
    pred = dict((k, v[0].cpu().numpy()) for k, v in pred.items())
    overlap_time = pred['overlap_time'] if 'overlap_time' in pred else 0.0
